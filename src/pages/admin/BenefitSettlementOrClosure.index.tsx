...
import { IAdminDataRequestParameters, IAdminItemTypes } from './admin.models';
...
import { getBenefitSettlementDataViewColumns } from './tableColumns.index';
...
import { attachBenefitWorkflowFN, setIsOneTimePayment } from './admin.thunk';
...
let initialBenefitSettlementPageSize = 12;
let totalRecords = 0;
...
const BenefitSettlementOrClosure: FC<any & {
  ...
  const [benefitSettlementPageSize, setBenefitSettlementPageSize] = useState(
    initialBenefitSettlementPageSize,
  );
  ...
  const updateCount = (res: AxiosResponse) => {
    ...
    if (
      pagDataKeyArr.indexOf('benefitSettlements') !== -1 ||
      item === 'benefit_settlement'
    ) {
      key = 'benefitSettlements';
      count =
        res?.data?.pagination_data?.benefitSettlements ||
    ...
    if (key !== '') {
      ...
      totalApprovedCount =
        res?.data?.pagination_data?.approved_benefits || count;
    }
  };
            ...
            >
              <Pagination
                ...
                onChange={(pageNumber: number, pageSize: any) => {
                  _pageSize = benefitSettlementPageSize;
                ...
                pageSize={benefitSettlementPageSize}
                showSizeChanger={true}
                onShowSizeChange={(_current: number, size: number) => {
                  setBenefitSettlementPageSize(size);
                }}
                total={totalRecords}
  ...
  const fetchSettlementStatus = async () => {
    try {
      const res: AxiosResponse = await getSettlementStatusService('BENFIT');
  ...
  const executeSettlement = async () => {
    if (item === 'benefit_settlement' || item === 'request_posting') {
      try {
        if (item === 'benefit_settlement')
          setSettlemenetStatus(predefinedSettlementStatuses.processing);

        const selectedAll =
          item === 'benefit_settlement' && isSettlementSelectedAllClicked
            ? // ||
        ...
        const ids =
          item === 'benefit_settlement' && !selectedAll
            ? settlementSelected
            : undefined;
        ...
        if (item === 'benefit_settlement') {
          message.success(
            'Benefit claims settlement has been initiated. Report will be available for download shortly.',
            1,
          );
  ...
  const fetchPreviousSettlements = async (allLogs?: any) => {
    try {
      let response: AxiosResponse = await getPreviousSettlementsService(
        (item === 'benefit_settlement' ? 'benefit' : 'request') as string,
        allLogs,
      );
      ...
      return (
        <p className='settlement-selection-instruction'>
          ...
          {!isAllSelected && (
            ...
            >{`Select all ${totalCount} items in Benefit Settlements.`}</Button>
          )}
        </p>
      );
    } catch (error) {
    ...
    <>
      <div className={`${item} settlement-items`}>
        <div className={filtersVisible ? 'active action-bar' : 'action-bar'}>
                ...
                >
                  <Button
                    ...
                    onClick={_event => {
                      ...
                      _setConfirmationInfo({
                        bodyText: `Out of ${totalApprovedCount} ${type}, ${selectedCount} ${isOrAre} going to settle.`,
                        cancelText: 'Cancel',
                        okText: 'Execute',
                        visibility: true,
                        extraInfo: '',
                  ...
                  >
                    <Trans>Benefit Settlements</Trans>
                  </Button>
                </ElementOrSkeleton>
          ...
          <DataFilter
            item={item === 'benefit_settlement' ? 'benefit' : item}
            // item={item}
            page='ADMIN'
      ...
      <Modal
        ...
        title={<Trans>Settlement Reports</Trans>}
        closable={true}
        onCancel={() => {
            ...
            <StatusTag
              ...
              onStatusClick={
                handleStatusTagClicked.bind(
                  ...
                  true,
                  drawerDetailItem?.itemType as 'benefit' | 'request',
                ) as any
              }
            />
          </>
      ...
      >
        {drawerDetailItem && (
          <BenefitDetail benefitClaimId={drawerDetailItem.id} isAdmin={true} />
        )}
      </AppDrawer>
    </>
  );
...
export default memo(connector(BenefitSettlementOrClosure));
