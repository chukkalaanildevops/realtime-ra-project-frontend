...
const ExpenseApprovedCard = (props: any) => {
  const {
    // variables //
    history,
    dashboardData,
  ...
  const allData = dashboardData[DASHBOARD_CARD_TYPES.approvedExpense];
  if (cardLoader.approvedExpense) {
    return <CardLoader title={DASHBOARD_CARD_TYPES.approvedExpense} />;
  } else {
    if (allData?.data?.length > 0) {
      ...
      return (
        <CardContainer
          ...
          title={DASHBOARD_CARD_TYPES.approvedExpense}
        >
          {data.map((dataItem: any, index: number) =>
            dataItem.showAllCard ? (
              <ShowAll
                ...
                type={DASHBOARD_CARD_TYPES.approvedExpense}
                status='APPRVD'
              />
            ) : (
              <CardItem
                ...
                _onRemarkAdded={() => _fetchSpecificData('approvedExpense')}
                onPrevClick={() =>
                  onAction(DASHBOARD_CARD_TYPES.approvedExpense, 'prev')
                }
                onNextClick={() =>
                  onAction(DASHBOARD_CARD_TYPES.approvedExpense, 'next')
                }
...
export default ExpenseApprovedCard;
